...
class UserServiceTest {
    ...
    public void checkPresentUserExists() {
    //Assertions.assertTrue(out.checkUserExist(name));
    Mockito.when(userDaoMock.getUserByName("Ivan")).thenReturn(Mockito.isNotNull());
    Assertions.assertNotNull(out.checkUserExist("Ivan"));
    }
    ...
    public void checkMissingUserDoesNotExist() {
    //Assertions.assertFalse(out.checkUserExist(name));
    Mockito.when(out.checkUserExist("Stepan")).thenReturn(Mockito.isNull());
    Assertions.assertFalse(out.checkUserExist("Stepan"));
    }
}
